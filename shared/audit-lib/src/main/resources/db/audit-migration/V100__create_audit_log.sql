...
-- Conditional CREATE SCHEMA: avoids the PG-15+ pg_namespace_nspname_index
-- duplicate-key race that can occur on `CREATE SCHEMA IF NOT EXISTS audit;`
-- when the catalog is contended (Flyway opens a fresh tx, the existence
-- check passes, then the unique index trips). We probe pg_namespace first
-- and only emit the create if the schema is genuinely missing.
DO $$
BEGIN
...
EXCEPTION WHEN duplicate_schema OR unique_violation THEN
    NULL; -- another session won the race
END
$$ LANGUAGE plpgsql;
