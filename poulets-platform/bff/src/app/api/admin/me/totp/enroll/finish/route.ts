...
/**
 * POST /api/admin/me/totp/enroll/finish
 * → self-service TOTP confirmation with 6-digit code.
 *
 * Self-management : tout admin authentifié peut finaliser son TOTP.
 *
...
import { TotpEnrollFinishSchema } from '@/lib/schemas/admin';

export async function POST(req: NextRequest) {
  const auth = await adminAuth(req, { requiredLevel: 'MANAGER' });
  if (!auth.ok) return auth.response;
  const parsed = TotpEnrollFinishSchema.safeParse(await req.json().catch(() => ({})));
  if (!parsed.success) {
  ...
  const res = await adminProxy(auth, {
    method: 'POST',
    target: 'auth-ms',
    path: '/admin/me/totp/enroll/finish',
    body: parsed.data,
    idempotencyKey,
  });
