...
/**
 * POST /api/admin/users/:userId/recovery-codes/generate
 * → génère 10 codes single-use ; renvoyés en clair UNE seule fois.
 *
 * Niveau requis : ADMIN.
 */
...
export async function POST(
  ...
  const res = await adminProxy(auth, {
    method: 'POST',
    path: `/admin/users/${encodeURIComponent(userId)}/recovery-codes/generate`,
    body: parsed.data,
    idempotencyKey,
  });
