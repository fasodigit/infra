...
/**
 * POST /api/admin/users/:userId/totp/enroll/finish
 * → confirme l'enrollement TOTP avec un code à 6 chiffres.
 *
 * Niveau requis : ADMIN.
 */
...
export async function POST(
  ...
  const res = await adminProxy(auth, {
    method: 'POST',
    path: `/admin/users/${encodeURIComponent(userId)}/totp/enroll/finish`,
    body: parsed.data,
    idempotencyKey,
  });
