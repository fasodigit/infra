...
/**
 * POST /api/admin/users/:userId/passkeys/enroll/finish
 * → finalise l'enrollement WebAuthn avec l'attestation client.
 *
 * Niveau requis : ADMIN.
 */
...
export async function POST(
  ...
  const res = await adminProxy(auth, {
    method: 'POST',
    path: `/admin/users/${encodeURIComponent(userId)}/passkeys/enroll/finish`,
    body: parsed.data,
    idempotencyKey,
  });
    ...
    void auditLog(
      {
        ...
        metadata: { method: 'passkey', deviceLabel: parsed.data.deviceLabel },
