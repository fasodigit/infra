...
//! Tenant context extraction for terroir-eudr.
//!
//! Mirrors `terroir-core::tenant_context` semantically. Extracts
//! `TenantContext` from `Authorization: Bearer <jwt>` (Kratos) or
//! `X-Tenant-Slug` header (M2M / internal calls).
...
impl TenantContext {
    ...
    pub fn evidence_bucket(&self, prefix: &str) -> String {
        format!("{prefix}-{}", self.slug)
    }
}

#[derive(Debug, Deserialize, Serialize)]
struct TerroirClaims {
    sub: String,
    #[serde(default)]
    role: String,
...
async fn extract_from_jwt(token: &str, state: &Arc<AppState>) -> anyhow::Result<TenantContext> {
        ...
        .unwrap_or(false)
    {
        ...
        let claims: TerroirClaims = serde_json::from_slice(&payload)?;
        let slug = claims
            .tenant_slug
    ...
    validation.set_audience(&["terroir-eudr"]);

    let token_data = jsonwebtoken::decode::<TerroirClaims>(token, &key, &validation)?;
